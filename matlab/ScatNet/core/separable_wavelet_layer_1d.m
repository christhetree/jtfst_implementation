...
%       coefficients for negative fr°j's, that is, chirplets going
...
function [U_Phi, U_Psi] = separable_wavelet_layer_1d(U, filters, scat_opt)
	...
	if calc_U
        ...
        S{1} = U_phi_tm;
        S{2} = U_phi_tm;
        U_phi_tm = wavelet_layer_1d(U_psi_tm, filters{2}, scat_opt);
        S{3} = U_phi_tm;
	...
	for s = 1
        ...
        for p1 = 1:length(Z.signal)
            ...
            scat_opt_fr.oversampling = 0;  % add
