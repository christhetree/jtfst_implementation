...
function [U_phi, U_psi] = wavelet_layer_1d_adapt(U, filters, scat_opt, wavelet)
	if nargin < 3
		scat_opt = struct();
	end
	
	if nargin < 4
		wavelet = @wavelet_1d; % you can choose the type of wavelets you want to use
	end
	...
	[psi_xi,psi_bw,phi_bw] = filter_freq(filters.meta); % get all the log centre frequency
	% only to dilate mother wavelet!
...
% 		psi_mask = calc_U&(current_bw>psi_xi); % only decompose high frequency parts?
		psi_mask = calc_U & ones(size(psi_xi)); % only decompose high frequency parts?
        % compare each scale bandwidth with filter bandwidth, only
        % decompose scales that are larger than filter bandwidth
        % which wavelets/filters will be used
        
        % frequency of the modulation must be lower than the frequency of
        % the signal itself
% 		psi_mask = calc_U&(current_bw>0);
        
		scat_opt.x_resolution = U.meta.resolution(p1);
		scat_opt.psi_mask = psi_mask;
		[x_phi, x_psi, meta_phi, meta_psi] = ... % wavelet transform
