...
function filters = morlet_filter_bank_1d(sig_length,options)
	...
	sigma0 = 2/sqrt(3); % spread of gaussian envelope of mother morlet filter
	...
	options = fill_struct(options, ...
		'B', options.Q);
	options = fill_struct(options, ...
		'xi_psi',1/2*(2^(-1/options.Q)+1)*pi);  % frequency of the oscillatory exponentials
% 		'xi_psi',1/2*(2^(-1/options.Q)+1)*pi);  % frequency of the oscillatory exponentials
	options = fill_struct(options, ...
		'sigma_psi',1/2*sigma0/(1-2^(-1/options.B))); % spread of gaussian envelope of morlet filter
	options = fill_struct(options, ...
	...
	filters.meta.psi_center = psi_center;
    ...
    filters.psi.meta.center = psi_center;
end
