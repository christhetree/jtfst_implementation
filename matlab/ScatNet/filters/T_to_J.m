...
function J = T_to_J(T, filt_opt)
   ...
   if  nargin ==2 && ~isstruct(filt_opt)
	...
	filt_opt = fill_struct(filt_opt,'phi_bw_multiplier', ...
		1+(filt_opt.Q==1));
%     filt_opt = fill_struct(filt_opt,'phi_bw_multiplier', 1);
%   original:
% 	J = 1 + round(log2(T./(1*filt_opt.B./filt_opt.phi_bw_multiplier)) ...
% 		.*filt_opt.Q);
	J = 1 + round(log2(T./(4*filt_opt.B./filt_opt.phi_bw_multiplier)) ...
		.*filt_opt.Q);
% 	J = 1 + round((log2(T./512)).*filt_opt.Q); % Flutter tonguing is longer than 10ms, at least contain one FT
    % 4 samples are used in the lowest scale/higest frequecy
end